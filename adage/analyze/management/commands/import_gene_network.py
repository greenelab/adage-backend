...
The first line will be ignored as it only has column names.  Here is a gzipped
example input file in this reop: data/gene_gene_network_cutoff_0.2.txt.gz

The command requires two arguments:
  (1) gene_network_file: the name of input gene-gene network file;
  (2) ml_model: the name of machine learning model for gene-gene network
...
whose machine leaning model is "Ensemble ADAGE 300", we will type:
  python manage.py import_gene_network <path>/eADAGE.txt "Ensemble ADAGE 300"

IMPORTANT:
Before running this command, please make sure that `ml_model` already exists
in the database.  If it doesn't, use `create_or_update_ml_model.py` command
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('gene_network_file', type=open)
        parser.add_argument('ml_model', type=str)

    def handle(self, **options):
        try:
            import_network(options['gene_network_file'], options['ml_model'])
            self.stdout.write(
...
def import_network(file_handle, ml_model_name):
    ...
    First validate input ml_model_name, then read each valid data line in the
    ...
    try:
        ...
        raise Exception("%s not found in the database" % ml_model_name)
...
def check_and_import(file_handle, ml_model):
    ...
    An exception will be raised when error is detected in file_handle.
    """
