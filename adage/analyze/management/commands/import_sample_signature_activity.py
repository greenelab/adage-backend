...
This management command can be invoked like this:
  ...
  python manage.py import_sample_signature_activity <activity_filename> <ml_model_name>

The two required arguments of this commands are:
  (1) activity_filename: a tab-delimited activity file's name;
  (2) ml_model_name: machine learning model's name that corresponds to activity_filename

IMPORTANT:
Before running this command, please make sure that ml_model_name already
exists in the database.  If it doesn't, use the management command
"create_or_update_ml_model.py" to add it to the database.
"""
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('activity_file', type=open)
...
def import_activity(file_handle, ml_model_name):
    ...
    try:
        ...
        raise Exception(
            "Input ml_model_name %s not exist in database" % ml_model_name
        )
...
def import_signatures(signatures, mlmodel):
    ...
    are detected:
      ...
      * Signature/ml_model_name pair is not unique.
    """
    ...
    for index, name in enumerate(signatures):
        if not name or name.isspace():
            ...
            raise Exception(
                "Input file line #1 column #%d: Signature name %s already exists "
                "in Signature table" % (index + 2, name)
            )
        else:
